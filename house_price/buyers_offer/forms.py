...
class BuyersOfferForm4(forms.Form):
    stove = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Stove")
    exceptions_1 = forms.CharField(max_length=100, required=False, label="Exceptions")
    refrigerators = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Refrigerator")
    exceptions_2 = forms.CharField(max_length=100, required=False, label="Exceptions")
    dryers_washers = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Dryers and Washers")
    exceptions_3 = forms.CharField(max_length=100, required=False, label="Exceptions")
    additional_items = forms.CharField(max_length=100, required=False, label="Additional Items")
    phone_automation_system = forms.BooleanField(widget=forms.CheckboxInput(), required=False, initial=True, label="Phone Automation System")
    not_owned_seller = forms.CharField(max_length=100, label="Not owned by Seller")
    buyer_primary_residence = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Buyer's Primary Residence")
    buyer_possession_1 = forms.CharField(max_length=100, required=False, label="Buyer's Possession")
    buyer_possession_2 = forms.CharField(max_length=100, required=False, label="Buyer's Possession")
    buyer_possession_3 = forms.CharField(max_length=100, required=False, label="Buyer's Possession")
    property_vacant_1 = forms.IntegerField(required=False, initial=5, label="Property Vacancy")
    property_vacant_2 = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Property Vacancy")
    condominium = forms.IntegerField(initial=7, label="Condominium")
    deliver_report = forms.IntegerField(initial=7, label="Deliver Report")
    inspection_contingency = forms.BooleanField(widget=forms.CheckboxInput(), required=False, label="Inspection Contingency")
    remove_inspection_contingency = forms.IntegerField(initial=17, required=False, label="Remove Inspection Contingency")
    property_access = forms.IntegerField(initial=17, label="Property Access")
    days_perform = forms.IntegerField(initial=2, label="Days to Perform")
    cancel_agreement = forms.IntegerField(initial=3, label="Cancel Agreement")
    final_verification = forms.IntegerField(initial=5, label="Final Verification")
    expiration_date = forms.DateField(label="Expiration Date")
    expiration_time = forms.CharField(max_length=100, label="Expiration Time")